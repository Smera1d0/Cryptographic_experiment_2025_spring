...
def genModel( Round, obj ):
    ...
    # 添加输入差分约束 - f000000000000000 (二进制: 1111 0000 0000 0000 ...)
    addConstr('ASSERT %s = 0bin1;' % X[0][0])
    addConstr('ASSERT %s = 0bin1;' % X[0][1])
    addConstr('ASSERT %s = 0bin1;' % X[0][2])
    addConstr('ASSERT %s = 0bin1;' % X[0][3])
    
    # 将其余位置为0
    for i in range(4, 64):
        addConstr('ASSERT %s = 0bin0;' % X[0][i])
